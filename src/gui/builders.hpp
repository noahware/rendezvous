...
namespace rv
{
	...
	#define RV_WIDGET_FACTORY_DEFS(CLS, GUI, PARENT)                                                  \
		inline button& CLS::add_button(const utf8_view text) {                                        \
		...
		inline checkbox& CLS::add_checkbox(const utf8_view label) {                                    \
		...
		inline text_element& CLS::add_label(const utf8_view text) {                                    \
		...
		inline text_element& CLS::add_label(const utf8_view text, shared_ptr_t<gui_font> font) {       \
			auto g = GUI; auto w = g->make_child<text_element>(PARENT, default_label_size(), font ? font : g->font()); \
			w->text_size(default_text_size(*g)); if (!text.empty()) w->content(text); return *w; }      \
		inline text_box& CLS::add_text_input(const utf8_view text) {                                   \
			auto g = GUI; auto w = g->make_child<text_box>(PARENT, default_input_size(), g->font(), g->get_input()); \
			w->text_size(default_text_size(*g)); if (!text.empty()) w->text(text); return *w; }        \
		inline text_box& CLS::add_text_area(const utf8_view text) {                                    \
		...
		inline plot_lines& CLS::add_plot_var(const utf8_view label, const float* value) {              \
			auto g = GUI; auto w = g->make_child<plot_lines>(PARENT, default_plot_size(), g->font(), g->get_input()); \
		...
		inline element& CLS::add_container(const utf8_view title) {                                    \
		...
		inline radio_button& CLS::add_radio_button(const utf8_view label, const cstd::int32_t value) { \
		...
		inline collapsing_header& CLS::add_collapsing_header(const utf8_view label) {                  \
