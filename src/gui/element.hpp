...
namespace rv
{
	...
	#define RV_WIDGET_FACTORY_DECLS                                                                 \
		button& add_button(utf8_view text = {});                                                  \
		checkbox& add_checkbox(utf8_view label = {});                                             \
		text_element& add_label(utf8_view text = {});                                             \
		text_element& add_label(utf8_view text, shared_ptr_t<gui_font> font);                      \
		text_box& add_text_input(utf8_view text = {});                                            \
		text_box& add_text_area(utf8_view text = {});                                             \
		...
		plot_lines& add_plot_var(utf8_view label, const float* value);                            \
		value_inspector& add_inspector();                                                         \
		element& add_row();                                                                       \
		element& add_column();                                                                    \
		element& add_container(utf8_view title = {});                                            \
		...
		radio_button& add_radio_button(utf8_view label = {}, cstd::int32_t value = 0);            \
		collapsing_header& add_collapsing_header(utf8_view label = {});                           \
	...
	public:
		...
		element& tooltip(const utf8_view text)
		{
			tooltip_ = utf8_string(text);
